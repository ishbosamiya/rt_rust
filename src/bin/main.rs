...
fn main() {
    let matches = clap::App::new("RT Rust")
        ...
        .arg(
            clap::Arg::with_name("quick_test")
                .long("quick_test")
                .help("A quick 3D testing scene"),
        )
        .get_matches();

    let run_headless = matches.is_present("headless");
    let run_quick_test = matches.is_present("quick_test");
...
/// Main function for doing quick and dirty 3D tests that require GUI
/// and a viewport
fn main_quick_test() {
    let mut glfw = glfw::init(glfw::FAIL_ON_ERRORS).unwrap();
